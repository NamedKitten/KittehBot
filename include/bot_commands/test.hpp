...
void test_command(json jmessage, Hexicord::Client& client) {
 std::string channel_id =
  ...
  client.sendRestRequest("POST", "/channels/" + channel_id + "/messages", {{"content", "```json\n" + msg.dump(4) + "\n```"}});

} else {
  client.sendRestRequest("POST", "/channels/" + channel_id + "/messages", {{"content", "Only the bots owner (" + application["owner"]["username"].get<std::string>() + ") can use this command."}});
