void serverinfo_command( json jmessage, json guilds ) {
 ...
 if ( !guild_json["icon"].is_null() ) {
  std::string icon_url = "https://cdn.discordapp.com/icons/" + gid + "/" +
                         guild_json["icon"].get<std::string>() + ".jpg";
 ...
 if ( roles.size() > 0 ) {
  ...
  //roles_str.reserve(((6 + 20) * roles.size()) + 1);
  ...
  if ( roles_str.length() > 1024 ) {
   em.add_field( "Roles", "Too many roles to list. (Is this Cheese.lab?)" );
  } else {
 ...
 if ( channels.size() > 0 ) {
  ...
  //channels_str.reserve(((5 + 20) * channels.size()) + 1);
  ...
  em.add_field( "Text channels", std::string( channels_str ) );
 }
 ...
 if ( !guild_json["afk_channel_id"].is_null() ) {
  ...
  info += "**AFK timeout**: " + std::to_string( guild_json["afk_timeout"].get<int>() ) +
          " seconds\n";
 }
